...
function computeCredibility(flags) {
  ...
  s -= penalize(flags.devTools || 0, 15, 20);
...
app.post('/api/sessions/:code/flag', (req, res) => {
  const sess = sessions[req.params.code.toUpperCase()];
  if (!sess) return res.status(404).json({ error: 'Session not found' });
  const { studentId, type, duration } = req.body;
  const student = sess.students[studentId];
  if (!student) return res.status(404).json({ error: 'Student not found' });

  // Support devTools as a new flag type
  if (!student.flags[type] && student.flags[type] !== 0) {
    student.flags[type] = 0;
  }
  student.flags[type]++;

  const ts = new Date().toLocaleTimeString('en-US', { hour12: false });
  const logEntry = { type, time: ts, question: student.answers.length + 1 };

  // Store duration for tab switches so teacher sees how long they were gone
  if (type === 'tabSwitch' && duration !== undefined) {
    logEntry.duration = duration;
    const durLabel = duration >= 60
      ? `${Math.floor(duration/60)}m ${duration%60}s`
      : `${duration}s`;
    logEntry.durationLabel = durLabel;
  }

  student.log.push(logEntry);
